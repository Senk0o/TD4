...
class DAOEmploye(val ss: SessionOracle) {
    ...
    fun create(e: employe) {
        ...
        try {
            val stmt: Statement = conn!!.createStatement()// Création d'une requete de type Statemen
            val result: Int= stmt.executeUpdate(requete) //Le contenu du select est dans ResultSet
        }
    ...
    fun update(e : employe) {
        var conn: Connection? = null
        conn= session?.getConnectionOracle()
        val requete: String="update employe set nuempl = ${e.getNuempl()}, nomempl = ${e.getNomempl()}, hebdo = ${e.getHebdo()}, affect = ${e.getAffect()}, salaire = ${e.getSalaire()})"
        try {
            val stmt: Statement = conn!!.createStatement()// Création d'une requete de type Statemen
            val result: Int= stmt.executeUpdate(requete) //Le contenu du select est dans ResultSet
        }
